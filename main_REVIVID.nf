...
main: 
if( params.cohort) {
genotypechrGVCFs(vcf.flatten().toList(),chromosomes_ch,params.genome,indexes_ch,params.broadinterval,params.genomedict,params.mask,params.ped)
combinechrVCFs(genotypechrGVCFs.out[0].toSortedList(),params.genome,indexes_ch,params.genomedict,params.ped)
variantcohortrecalibration(combinechrVCFs.out[0].toSortedList(),combinechrVCFs.out[1].toSortedList(),params.genome,params.genomedict,indexes_ch,params.snps,params.snpsindex,params.snpstruth,params.snpstruthindex,params.indels,params.indelsindex,params.ped)
...
SelectVariantsdenovo(createfamilyvcfs.out.map{fam,caller,vcf,vcfindex -> tuple(fam,caller,vcf)},params.genome,params.genomedict,indexes_ch,params.ped,params.mask)
SelectVariantsAR(createfamilyvcfs.out.map{fam,caller,vcf,vcfindex -> tuple(fam,caller,vcf)},params.genome,params.genomedict,indexes_ch,params.ped,params.mask)
SelectVariantsX(createfamilyvcfs.out.map{fam,caller,vcf,vcfindex -> tuple(fam,caller,vcf)},params.genome,params.genomedict,indexes_ch,params.ped,params.mask)
SelectVariantsAR.out[0].map{fam,analysis,mode,vcfgz,tbi -> tuple(fam,analysis,vcfgz,tbi)}.groupTuple(by:1).flatten().unique().view()
//mergevcf(SelectVariantsAR.out[0].map{fam,analysis,mode,vcfgz,tbi -> tuple(fam,analysis,vcfgz,tbi)}.groupTuple(by:1).flatten().unique().collate( 8 ))
annotatedenovo(SelectVariantsdenovo.out[0],params.programs,params.humandb,params.annovardbs)
