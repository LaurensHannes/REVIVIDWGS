...
readgroups.out.flatten().collate( 4 ).map{id,lane,bam,bai -> tuple(id,bam,bai)}.join(duplicates.out[0].flatten().collate ( 2 ).map{id,bam -> tuple(id,bam)}).join(mergebams.out[0].flatten().collate ( 3 ).map{id,bam,bai -> tuple(id)}).join(generateCRAM.out[0].flatten().collate ( 3 ).map{id,cram,crai -> tuple(id)}).dump(tag:"garbage5").set{testgarbage_ch4}
...
testcollection.concat(testgarbage_ch,testgarbage_ch2,testgarbage_ch3,testgarbage_ch4).dump(tag:"G12345").set{concatedtestcollection}
