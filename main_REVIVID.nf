...
importfastq.out.flatten().filter(~/.*R\d+.*.fastq.gz/).map{file -> tuple(file.getBaseName(3), file)}.groupTuple().dump(tag:"test").flatten().collate( 3 ).map{R1,R2,lane -> tuple(R1.simpleName,lane,R1,R2)}.set{gzipped_ch}
