...
readgroups.out.flatten().collate( 4 ).map{id,lane,bam,bai -> tuple(id,bam,bai)}.join(duplicates.out[0].flatten().collate ( 2 ).map{id,bam -> tuple(bam.getBaseName(2))}).dump(tag:"garbage4").set{testgarbage_ch4}
