...
process mergevcf {
	...
	tag "merging family:${fam} vcfs for analysis"
	cpus 4
	...
	output:
	tuple val(fam),val(analysis),val("merged"),file("${fam}.${analysis}.merged.vcf.gz"), file("${fam}.${analysis}.merged.vcf.gz.tbi")
	
"""
	bcftools concat -a -o ${fam}.${analysis}.merged.vcf.gz -O z --threads ${task.cpus} $vcf1 $vcf2 $vcf3
...
process subsetvcf {
	...
	output: 
	tuple val(fam), val(caller), file("${fam}.${caller}.${id}.subset.vcf.gz"), file("${fam}.${caller}.${id}.subset.vcf.gz.tbi")
	
	"""
	bcftools view -a -O z -s $id,$father,$mother $vcfgz > ${fam}.${caller}.${id}.subset.vcf.gz 
	tabix ${fam}.${caller}.${id}.subset.vcf.gz
	"""
}