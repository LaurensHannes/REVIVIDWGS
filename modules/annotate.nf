...
process VEP {
		...
		tuple val(family), val(analysis), val(mode) ,file("${family}.${analysis}.${mode}.hg38_VEP.vcf") 
		tuple val(family), val(analysis), val(mode) ,file("${family}.${analysis}.${mode}.hg38_VEP.txt_summary.html")
        """
		vep -i $vcfgz --cache --dir ${VEPpath} --fork ${task.cpus}  --check_frequency --freq_pop 1KG_EUR --freq_gt_lt gt --freq_freq 0.01 --freq_filter exclude -o ${family}.${analysis}.${mode}.hg38_VEP.txt --af_gnomadg --hgvs --show_ref_allele --symbol --pick --regulatory --no_intergenic --canonical --gene_phenotype --pubmed --plugin NMD --plugin pLI,${VEPpath}/pLI_values.txt --plugin REVEL,${VEPpath}/new_tabbed_revel_grch38.tsv.gz --tab --plugin AlphaMissense,file=${VEPpath}/AlphaMissense_hg38.tsv.gz --plugin SpliceAI,snv=${VEPpath}/spliceai_scores.raw.snv.hg38.vcf.gz,indel=${VEPpath}/spliceai_scores.raw.indel.hg38.vcf.gz
		vep -i $vcfgz --cache --dir ${VEPpath} --fork ${task.cpus}  --check_frequency --freq_pop 1KG_EUR --freq_gt_lt gt --freq_freq 0.01 --freq_filter exclude -o ${family}.${analysis}.${mode}.hg38_VEP.vcf --af_gnomadg --hgvs --show_ref_allele --symbol --pick --regulatory --no_intergenic --canonical --gene_phenotype --pubmed --plugin NMD --plugin pLI,${VEPpath}/pLI_values.txt --plugin REVEL,${VEPpath}/new_tabbed_revel_grch38.tsv.gz --vcf --no_stats --plugin AlphaMissense,file=${VEPpath}/AlphaMissense_hg38.tsv.gz --plugin SpliceAI,snv=${VEPpath}/spliceai_scores.raw.snv.hg38.vcf.gz,indel=${VEPpath}/spliceai_scores.raw.indel.hg38.vcf.gz
		"""
}